...
const NAV = [
  { href: "#system", label: "The system", short: "System" },
  { href: "#confarena", label: "ConfArena", short: "Arena" },
  { href: "#results", label: "Results", short: "Results" },
  { href: "#validation", label: "Validating ConfArena", short: "Checks" },
  { href: "#decks", label: "The decks", short: "Decks" },
];

/**
 * Masthead — a running head: the system name, where you are in the argument,
 * and the one thing most visitors came for.
 */
export default function Masthead() {
  ...
  useEffect(() => {
    const sections = NAV.map((item) => document.querySelector<HTMLElement>(item.href)).filter(
      Boolean,
    ) as HTMLElement[];
    let frame = 0;
  ...
  return (
    <header className={`masthead${pastHero ? " is-detached" : ""}`}>
      <div className="masthead-inner">
        <a href="#top" className="masthead-name" aria-label="SlideLab, back to top">
        ...
        <nav className="masthead-nav" aria-label="Sections">
          {NAV.map((item, index) => {
            ...
            return (
              ...
              >
                <span className="masthead-nav-number" aria-hidden="true">
